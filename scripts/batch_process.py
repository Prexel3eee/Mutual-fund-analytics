"""Batch process all SBI MF portfolio Excel files in a directory."""
import argparse
import json
import logging
import os
...
# Import the ETL runner from sbi_etl
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))
from src.sbi_etl import run_etl
...
def main():
    parser = argparse.ArgumentParser(
        description="Batch process SBI MF portfolio files"
    )
    parser.add_argument(
        "input_dir",
        help="Directory containing Excel files (e.g., ./2025)",
    )
    parser.add_argument(
        "--output-dir", "-o",
    ...
    parser.add_argument(
        "--pattern", "-p",
        help="Filename pattern to match (default: *.xlsx)",
        default="*.xlsx",
    )
    ...
    excel_files = sorted(input_path.glob(args.pattern))
    excel_files = [f for f in excel_files if not f.name.startswith("~$")]  # Skip temp files
    
    if not excel_files:
        log.error(f"No Excel files found matching pattern '{args.pattern}' in {input_path}")
        sys.exit(1)
    ...
    for idx, excel_file in enumerate(excel_files, 1):
        ...
        try:
            ...
            # Determine output filename based on report date
            ...
            output_file = output_path / f"sbi_equity_holdings_{date_suffix}.json"
